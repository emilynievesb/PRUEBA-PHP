...
{
    private $queryPost = 'INSERT INTO region (nombreReg, idDep) VALUES (:nombre_reg,:id_dep)';
    private $queryPut = 'UPDATE region SET nombreReg = :nombre_reg, idDep =:id_dep WHERE  idReg = :id_reg';
    private $queryGetAll = 'SELECT  idReg AS "id_reg", nombreReg AS "nombre_reg", idDep AS "id_dep" FROM region';
    private $queryDelete = 'DELETE FROM region WHERE idReg = :id_reg';
    private $message;

    use getInstance;
    //*Se definen el tipo de dato: static, private, public
    function __construct(private $id_reg = 1, public $nombre_reg = 1, private $id_dep = 1)
    {
        parent::__construct();
    ...
    public function post_region()
    {
        ...
        try {
            ...
            $res->bindValue("nombre_reg", $this->nombre_reg);
            $res->bindValue("id_dep", $this->id_dep);
